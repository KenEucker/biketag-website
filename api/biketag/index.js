...
const sendEmailToAllAdministrators = async (config, subject, body, latestTagInfo) => {
	...
	if ((!subject && !body) && !latestTagInfo) {
		await getTagInformation(config, 'latest', config.imgur.imgurAlbumHash, (t) => latestTagInfo = t)
	}
	...
	body = body || `Hello BikeTag Admin, this email is a confirmation that the most recent tag, number ${latestTagNumber}, has been autposted to reddit by BikeTag.Org.\r\n\r\nYou are getting this email because you are listed as an admin on the site (${config.host}).\r\n\r\nReply to this email to request to be removed from this admin list.`

	config.adminEmailAddresses.forEach((emailAddress) => {
		emailPromises.push(_app.sendEmail(config, {to: emailAddress, subject, text: body, callback: (info) => {
...
const postLatestBikeTagToReddit = (config, callback) => {
	const redditOpts = {
		userAgent: config.reddit.redditUserAgent.replace('VERSION', config.version).replace('biketag.org', config.host),
		...
		try {
			...
			return getTagInformation(subdomainConfig, 'latest', subdomainConfig.imgur.imgurAlbumHash, (latestTagInfo) => {
				...
				subdomainConfig.adminEmailAddresses.forEach((emailAddress) => {
					app.sendEmail(subdomainConfig, { emailAddress, subject, body, callback: (info) => {
						console.log(`email sent to ${emailAddress}`, info)
