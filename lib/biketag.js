...
class BikeTagApi {
    ...
    getBikeTagUsernameFromImage(image) {
		...
		if (image.description) {
			const split = image.description.indexOf('by') !== -1 ? image.description.split('by') : image.description.split('for')
	...
	/// TODO: cache this response
    getBikeTagsByUser(imgurClientID, albumHash, username, callback) {
        return this.getBikeTagImages(imgurClientID, albumHash, (images) => {
			if (username) {
				...
				const usernames = [], imagesGroupedByUsername = {}
				...
				usernames.forEach((username) => {
					imagesGroupedByUsername[username] = sortedImages.filter(i => this.getBikeTagUsernameFromImage(i).localeCompare(username) === 0)
				})
