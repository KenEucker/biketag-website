...
const getTagInformation = (config, tagNumber, albumHash, callback) => {
    const getTagRequest = _getImgurAlbumInfo(config, albumHash, (data) => {
        const images = getImagesByTagNumber(data.images)
...
const getBikeTagNumberFromImage = (image) => {
    let tagNumber
...
const getImagesByTagNumber = (images) => {
...
const getImagesByUploadDate = (images, newestFirst) => {
    if (!newestFirst) {
        return images.sort(
...
const getTagNumberIndex = (images, tagNumber, proof = false) => {
...
module.exports = {
    getTagInformation,
    postLatestBikeTagToReddit,
    getBikeTagNumberFromImage,
    getImagesByTagNumber,
    getImagesByUploadDate,
    getTagNumberIndex,
}
