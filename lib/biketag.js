...
class BikeTagApi {
    ...
    getBikeTagImages(imgurClientID, albumHash, callback, uncached = false) {
        ...
        if (!getBikeTagImagesResponse || uncached) {
            ...
            return this.getImgurAlbumInfo(imgurClientID, albumHash, (data) => {
                ...
                console.log('got new getBikeTagImages', { cacheKey, image: { images } })
        ...
        if (!getTagInformationResponse || uncached) {
            ...
            return this.getBikeTagImages(
                ...
                (images) => {
					...
					console.log('looking for tag', tagNumber)

                    if (nextTagIndex == -1) {
						console.log({images, nextTagIndex, prevTagIndex, previousMysterTagIndex})
                        return callback(null)
                    }
                    const imgurBaseUrl = 'https://imgur.com'
					const nextTagURL = images[nextTagIndex].link
					const nextTagURLExt = extname(nextTagURL)
                    const nextTagURLThumb = images[nextTagIndex].link.replace(nextTagURLExt, `m${nextTagURLExt}`)
                    ...
                    const tagData = {
						...
						proofText: null,
						proofTagURL: null,
						proofTagURLDirect: null,
						credit: null,
						previousMysterTagImage: null,
						previousMysteryTagURL: null,
                    }

					if (prevTagIndex !== -1) {
						const prevTagImage = images[prevTagIndex]
						const proofText = prevTagImage.description
						const split = proofText.indexOf('for') !== -1 ? proofText.split('for') : proofText.split('by')

						tagData.proofTagURL = `${imgurBaseUrl}/${prevTagImage.id}`
                        tagData.proofTagURLDirect = prevTagImage.link
						tagData.credit = split[split.length - 1].trim()
						tagData.proofText = proofText

						const previousMysterTagImage = previousMysterTagIndex !== -1 ? images[previousMysterTagIndex] : {}
						tagData.previousMysterTagImage = previousMysterTagImage
                        tagData.previousMysteryTagURL = previousMysterTagImage.link
					}
    ...
    postCurrentBikeTagToReddit(config, callback) {
        ...
        return this.getTagInformation(
            ...
            async (tagData) => {
                ...
                return await r
                    ...
                    .submitSelfpost({
                        title: `Bike Tag #${config.currentTagNumber}`,
                        text: currentTagTemplate,
                    })
                    .assignFlair({
                    ...
                    .submitCrosspost({
                        ...
                        title: `[X-Post r/${config.reddit.subreddit}] Bike Tag #${config.currentTagNumber} (${config.region})`,
                        resubmit: false,
                    })
